/*
    ...
    How to know if drawing a sprite or background/window tile??
*/
...
impl Render {
    ...
    pub fn update_screen(self: &mut Self, mem: &Memory) {}

    // result is a vector of 64 bytes. Each byte is a pixel
    ...
    fn calculate_addr(tile_no: u8, mem: &Memory) -> u16 {
        ...
        let addr: u16 = match is_sprite {
            ...
            false => {
                let ldlc_b4 = Render::get_ldlc_b4(mem);
                match ldlc_b4 {
                    true => 0x8000 + (u16::from(tile_no as u8) * 16),
                    false => {
    ...
    fn get_ldlc_b4(mem: &Memory) -> bool {
        return if ((mem.read_byte(LDLC_REG) >> 4) & 0x01) == 0x01 {
            true
        } else {
            false
        };
...
fn test_get_ldlc_b4() {
    ...
    mem.write_bytes(LDLC_REG, Vec::from([0x07]));
    assert_eq!(Render::get_ldlc_b4(&mem), false);

    mem.write_bytes(LDLC_REG, Vec::from([0xFF]));
    assert_eq!(Render::get_ldlc_b4(&mem), true);

    mem.write_bytes(LDLC_REG, Vec::from([0xEF]));
    assert_eq!(Render::get_ldlc_b4(&mem), false);

    mem.write_bytes(LDLC_REG, Vec::from([0x0F]));
    assert_eq!(Render::get_ldlc_b4(&mem), false);
}
...
fn test_weave_tile_from_index_b4_as_1() {
    let mut mem = Memory::new();
    mem.write_bytes(LDLC_REG, Vec::from([0x17]));
